...
public class VNMOrderRoutingImpl extends OrderRoutingImpl
{
	...
	public void validateBalancePostpaid(OrderRoutingInstance instance,
			...
			throws Exception
	{
		...
		ProductPrice productPrice = product
				.getProductPrice(vnmMessage.getChannel(), vnmMessage
						.getActionType(), vnmMessage.getSegmentId(), vnmMessage
						.getAssociateProductId(), vnmMessage.getQuantity(), vnmMessage
						.getOrderDate());
		...
		vnmMessage.setQuantity(quantity);
		vnmMessage.setAmount(vnmMessage.getPrice() * vnmMessage
				.getQuantity());
		...
		else
		{
			try
			{
				...
				try
				{
					...
					instance.logMonitor("SEND:" + strRequest + ". ID="
							+ sessionId);
				...
				catch (Exception e)
				{
					...
					validateBalancePostpaid(instance, orderRoute, product, vnmMessage);
				}
				finally
				{
				...
				else
				{
					...
					{
						// Set subscriber type
						if (vnmMessage.getSubscriberType() == Constants.UNKNOW_SUB_TYPE)
						{
							order.setSubscriberType(SubscriberEntryImpl
									.getSubscriberType(order.getIsdn()));
						}

						vnmMessage.setAmount(order.getQuantity() * order.getPrice());
					}
					else
					{
	...
	public CommandMessage checkSubType(OrderRoutingInstance instance,
			ProductRoute orderRoute, CommandMessage order) throws Exception
	{
		...
		else
		{
			try
			{
				...
				// get subscriber information in CCWS
				int queryLevel = orderRoute.getParameters()
						.getInteger("prepaid.queryLevel", 1);
				...
				try
				{
					...
					{}
					String strRequest = (new CCWSCommandImpl())
							.getLogRequest("com.comverse_in.prepaid.ccws.ServiceSoapStub.retrieveSubscriberWithIdentityNoHistory", vnmMessage
									.getIsdn());
					instance.logMonitor("SEND:" + strRequest + ". ID=" + sessionId);
					Date startTime = new Date();
					subscriberRetrieve = connection.getSubscriber(vnmMessage
							.getIsdn(), queryLevel);
					Date endTime = new Date();
					long costTime = CommandUtil
							.calculateCostTime(startTime, endTime);
					if (subscriberRetrieve != null)
					{
						subscriberEntity = subscriberRetrieve
								.getSubscriberData();
						String strResponse = (new CCWSCommandImpl())
								.getLogResponse(subscriberEntity, vnmMessage
										.getIsdn());
				...
				else
				{
					...
					{
						vnmMessage
								.setResponseValue(balance.getBalanceName() + ".amount", StringUtil
										.format(balance.getBalance(), "#"));
						vnmMessage
								.setResponseValue(balance.getBalanceName() + ".expireDate", StringUtil
										.format(balance.getAccountExpiration()
												.getTime(), "dd/MM/yyyy HH:mm:ss"));
					}

					vnmMessage
							.setResponseValue(ResponseUtil.SERVICE_PRICE, StringUtil
