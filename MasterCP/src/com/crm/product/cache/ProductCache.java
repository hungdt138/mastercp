...
public class ProductCache
{
	...
	protected void loadProducts(Connection connection) throws Exception
	{
		...
		try
		{
			...
			while (rsConfig.next())
			{
				ProductEntry product = new ProductEntry(rsConfig.getLong("productId"), rsConfig.getString("code"));

				product.setProductType(Database.getString(rsConfig, "Type"));
				product.setTitle(Database.getString(rsConfig, "title"));

				product.setAlias(Database.getString(rsConfig, "code"));
				...
				product.setSubscriberTypes(rsConfig.getString("subscriberModes"));
	...
	protected synchronized void loadProductPrice(Connection connection) throws Exception
	{
		...
		try
		{
			...
			while (rsConfig.next())
			{
				...
				price.setRelateProductId(rsConfig.getLong("linkProductId"));

				// price.setCurrency(rsConfig.getString("currency"));
				price.setCurrency("VND");
				price.setFlatCharge(rsConfig.getBoolean("methodType"));
