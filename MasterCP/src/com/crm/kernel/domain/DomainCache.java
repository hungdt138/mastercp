...
public class DomainCache
{
	...
	protected void loadDomain(Connection connection) throws Exception
	{
		...
		try
		{
			String sql = "Select * From AppDomain A Order by code desc";
			...
			while (rsConfig.next())
			{
				log.debug(String.format("Caching domain rule for %s ...", rsConfig.getString("code")));

				DomainEntry domain = new DomainEntry(rsConfig.getLong("domainId"),rsConfig.getString("code"));

				domain.setDomainType(Database.getString(rsConfig, "type"));
				...
				log.debug(String.format("Domain rule %s is cached", rsConfig.getString("code")));
			}
		}
		catch (Exception e)
		{
