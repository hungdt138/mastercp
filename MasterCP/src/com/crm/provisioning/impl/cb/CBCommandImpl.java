...
public class CBCommandImpl extends CommandImpl
{
	...
	/*
	 * HUNGDT
	 * Check status postpaid
	 */
	public void validatePostpaidStatus(CommandInstance instance,
			ProvisioningCommand provisioningCommand, VNMMessage vnmMessage, Properties prtResponse)
			throws Exception
	{
		CBConnection connection = null;
		...
		{	
			...
			long sessionId = setRequest(instance, vnmMessage, getLogRequest("com.crm.provisioning.impl.cb.CBCommandImpl ", vnmMessage.getIsdn()));
			
			long status = connection.getPostpaidStatus(vnmMessage.getIsdn(), prtResponse);
			
			//vnmMessage.setResponse("Status = " + status +", activeDate =  " +prtResponse.getProperty("active_date")+ " {"+vnmMessage.getIsdn()+"}");
			
			setResponse(instance, vnmMessage, "Status = " + status +", activeDate =  " +prtResponse.getProperty("active_date")+ ", Amount=" + vnmMessage.getAmount() + ", MerchantId = " + vnmMessage.getMerchantId(), sessionId);
			
			if (status == -1)
			{
				throw new AppException(Constants.ERROR_SUBSRIBER_NOT_ACTIVE);
			}
		}
		catch (Exception e)
		{
	...
	public VNMMessage checkPostpaidStatus(CommandInstance instance,
			ProvisioningCommand provisioningCommand, CommandMessage request) throws Exception
	{
		...
		try
		{
			...
			try
			{
				...
				validatePostpaidStatus(instance, provisioningCommand, vnmMessage, prtResponse);
				
				if(vnmMessage.getParameters().getInteger(Constants.REQUEST_PREFIX + "chargeMode") == 0) 
				{
					validateBalance(instance, orderRoute, product, vnmMessage);
				}
				
				SubscriberOrderImpl.updateExportStatus(Constants.EXPORT_STATUS_PENDING, request.getOrderId());
			}
			catch (AppException e)
			{
				if (e.getMessage().equals(Constants.ERROR_SUBSRIBER_NOT_ACTIVE))
				{
					isLock = true;
				}
				else
			...
			if(isLock)
			{
				...
				String cause = Constants.ERROR_SUBSRIBER_NOT_ACTIVE;
			...
			else
			{
				...
				if (actionType.equals(Constants.ACTION_SUBSCRIPTION))
				{
					...
					if (graceDate.compareTo(currentDate) < 0)
					{
						...
						cause = Constants.ERROR_SUBSCRIPTION;
					}
					
					Date activeDate = sdf.parse(prtResponse.getProperty("active_date"));
					
					if(activeDate.after(registerDate))
					{	
						vnmMessage.setActionType(Constants.ACTION_CANCEL);
						vnmMessage.setResponse("ActiveDate after registerDate");
						cause = Constants.ERROR_SUBSCRIBER_CHANGE;
						instance.debugMonitor("ActiveDate after registerDate");
					}
