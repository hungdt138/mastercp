...
public class SMPPConnection extends ProvisioningConnection {
	...
	public boolean openConnection() throws Exception {
		try {
			...
			registeredDelivery = parameters.getByte("registeredDelivery",
					Byte.valueOf("0"));
	...
	public boolean validate() throws Exception {
		...
		if (hasError)
			return false;
		Date now = new Date();
		if (now.getTime() - lastCheck.getTime() < enquireInterval) {
			return true;
		}
	...
	public void setAddressParameter(String descr, Address address, byte ton,
			byte npi, String addr) throws Exception {
		address.setTon(ton);
		address.setNpi(npi);
	...
	public synchronized void submit(CommandMessage request) throws Exception {
		try {
			int smsType = Integer.parseInt(request.getResponseValue(
					ResponseUtil.SMS_TYPE, SMS_TYPE_TEXT + ""));
	...
	public void submitWSPMessage(CommandMessage request) throws Exception {
		...
		try {
			...
			//PushMessage pushMessage = new PushMessage(href, text, ENC_GSM7BIT);
	 ...
	 * @throws Exception
	 */
	private SubmitSM createSubmitSM(CommandMessage request,
			ByteBuffer headerBuffer, ByteBuffer contentBuffer, byte dataCoding)
			throws Exception {
		...
		SubmitSM submitRequest = createSubmitFromRequest(request, useHeader,
				dataCoding);
		...
		if (null != messageBuffer && messageBuffer.length() > MAX_SMS_LENGTH) {
	 ...
	 * @throws Exception
	 */
	private SubmitSM[] createConcatenatedSubmitSMs(CommandMessage request,
			ByteBuffer headerBuffer, ByteBuffer contentBuffer, byte dataCoding)
			throws Exception {
		int total = 0;
		if (headerBuffer == null) {
		...
		for (int index = 1; index <= total; index++) {
			submitRequests[index - 1] = createSubmitFromRequest(request, true,
					dataCoding);
	 ...
	 */
	private SubmitSM createSubmitFromRequest(CommandMessage request,
			boolean useUserDataHeader, byte dataCoding)
	 ...
	 * @throws Exception
	 */
	private void submit(CommandMessage request, ByteBuffer headerBuffer,
			ByteBuffer contentBuffer, byte dataCoding) throws Exception {
		int msgLength = contentBuffer.length();

		if (headerBuffer != null)
			msgLength += headerBuffer.length();

		if (msgLength > MAX_SMS_LENGTH & useConcatenated) {
			SubmitSM[] submitRequests = createConcatenatedSubmitSMs(request,
					headerBuffer, contentBuffer, dataCoding);
			for (SubmitSM submitRequest : submitRequests) {
				getDispatcher().debugMonitor(
						"Submit concatenated request "
								+ submitRequest.debugString());
				submit(submitRequest);
			}
		} else {
			// prepare SMPP request
			SubmitSM submitRequest = createSubmitSM(request, headerBuffer,
					contentBuffer, dataCoding);
			getDispatcher().debugMonitor(
					"Submit request " + submitRequest.debugString());
			submit(submitRequest);
		}
	}
	...
	public boolean enquireLink() throws Exception {
		try {
			...
			if (mblnAsynchronous) {
				logMonitor("Local queue size: " + getTempQueueSize());
				...
				debugMonitor("Reponse: async response received "
	...
	public int getTempQueueSize() {
		if (pduListener != null)
			return pduListener.getTempQueueSize();
		else
			return 0;
	}
			...
			if (pdu == null) {
				...
				try {
					if (pdu instanceof DeliverSM) {
						...
						sms.setServiceAddress(deliverSM.getDestAddr()
								.getAddress());
						// sms.setShipTo(deliverSM.getSourceAddr().getAddress());

						sms.setRequest(deliverSM.getShortMessage()
								.toUpperCase());
						sms.setKeyword(deliverSM.getShortMessage()
								.toUpperCase());
	...
	private void initParameter() throws Exception {
		...
		setAddressParameter("source-address", sourceAddress, sourceTON,
				sourceNPI, mstrSourceAddress);

		setAddressParameter("destination-address", destAddress, destTON,
				destNPI, mstrDestAddress);
	}
	...
	public static int countSMS(String smsContent)
		...
		int total = (smsContent.length() / MAX_SMS_LENGTH) + 1;
