 ...
 * -------------------------------------
 */
package com.crm.provisioning.impl.vms.otp;
...
public class OTPCommandImpl extends CommandImpl
{
			...
			ProvisioningCommand provisioningCommand, CommandMessage request) throws Exception
	{
		...
		try
		{
			...
			long sessionId = setRequest(instance, request, getLogRequest("com.crm.provisioning.impl.vms.otp.OTPCommandImpl.OTPRequest ", result.getIsdn(), result.getMerchantId(), result.getProductId(), action));
			int response = connection.createOTPRequest(isdn, request.getRequestId(), productCode, action);
			...
			ProvisioningCommand provisioningCommand, CommandMessage request) throws Exception
	{
		...
		try
		{
			...
			long sessionId = setRequest(instance, request, getLogRequest("com.crm.provisioning.impl.vms.otp.OTPCommandImpl.SubRequest ", result.getIsdn(), result.getMerchantId(), result.getProductId(), otp));
			int response = connection.createSUBRequest(isdn, request.getRequestId(), otp);
