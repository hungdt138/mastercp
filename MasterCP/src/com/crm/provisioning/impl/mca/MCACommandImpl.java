...
public class MCACommandImpl extends CommandImpl {
									...
									CommandMessage order, String prefix ) throws Exception
	{
		...
		try{
			...
			// Debug
			instance.getDispatcher().logMonitor(template);
			// End debug
		...
		try
		{
			...
			else
			{
				...
				while(true)
				{
					...
					instance.getDispatcher().logMonitor("template ====="+ template);
			...
			order.setResponse(responseCode);
			instance.getDispatcher().logMonitor("end ...  MCA at excuteMUtilCommand");
			result = SUCCESS_RESPONSE;
	...
	public VNMMessage registerService(CommandInstance instance,ProvisioningCommand provisioningCommand, CommandMessage order ) throws Exception
	{
		...
		{
			int resultRespone = executeMutilCommand(instance, provisioningCommand, order, order.getProvisioningType());
	...
	public VNMMessage barring(CommandInstance instance, ProvisioningCommand provisioningCommand, CommandMessage order) throws Exception
	{
		...
		{
			int resultResonse = executeMutilCommand(instance, provisioningCommand, order, order.getProvisioningType());
	...
	public VNMMessage getHistory(CommandInstance instance, ProductRoute orderRoute, CommandMessage order) throws Exception
	{
		VNMMessage result = CommandUtil.createVNMMessage(order);
		String language="";
		String isdn = order.getIsdn();
		...
		_mcaConn = (MCAConnection) instance.getConnection();
		try
		{
			...
			String sql = "SELECT "
					+ "CALLINGPARTY" + ","
					+ "SUM(NOOFTIMES) as NOOFTIMES " + ","
					+ "MAX(IAMTIMESTAMP) as IAMTIMESTAMP " +
				"FROM MCA_ALL_HISTORY " +
				"WHERE " +
					"IAMTIMESTAMP > trunc(sysdate -3) AND " +
					"NOOFTIMES > 0 AND " +
					"REPLACE <> 3 AND " +  // Add DUYMB 2011/05/05 Confirm by Mr Luc.
					"MSISDN = ?  GROUP BY CALLINGPARTY ";
			preStatement = _connection.prepareStatement(sql);
			preStatement.setString(1, isdn);
			resulSet = preStatement.executeQuery(sql);
			language = getLanguage(isdn, _mcaConn);
			
			String history = getProperty("header." + language, "");
			String dateFormat = getProperty("dateFormat", "yyyy-MM-dd HH:mm:ss");

			String lineFormat =
					getProperty(
							"inline." + language
							, "<calling>: <times> lan, cuoc goi cuoi: <lastcall>.");
			while (resulSet.next())
			{
