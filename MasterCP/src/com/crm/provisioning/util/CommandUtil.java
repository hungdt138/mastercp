...
public class CommandUtil
{
	public static String addCountryCode(String isdn)
	{
		...
		{
			isdn = Constants.COUNTRY_CODE
					+ isdn.substring(Constants.DOMESTIC_CODE.length());
		}
	...
	public static String addDomesticCode(String isdn)
	{
		...
		if (!isdn.startsWith(Constants.DOMESTIC_CODE))
		{
			...
			{
				isdn = Constants.DOMESTIC_CODE
						+ isdn.substring(Constants.COUNTRY_CODE.length());
			}
			else
			{
	...
	public static String getCause(CommandMessage request)
	{
		if (request != null)
		{
			...
			if (cause.equals(""))
			{
				cause = (request.getStatus() == Constants.ORDER_STATUS_DENIED) ? Constants.ERROR
						: Constants.SUCCESS;
			}
	...
	public static boolean isTimeout(Date requestTime, int timeout)
			throws Exception
	{
	...
	public static boolean isTimeout(CommandMessage request, int timeout)
			throws Exception
	{
	...
	public static void processError(CommandInstance instance,
			CommandMessage request, String errorCode)
	{
		if (!errorCode.equals(""))
		{
	...
	{
		long timeCost = (endTime != null ? endTime.getTime() : 0)
				- (startTime != null ? startTime.getTime() : 0);
		//
	...
	public static CommandMessage sendSMS(DispatcherInstance instance,
			...
			String content) throws Exception
	{
		...
		try
		{
			...
			if (serviceAddress.equals(""))
			{
				...
				if (product != null)
				{
					serviceAddress = product.getParameter("serviceAddress",
	...
	public static void sendSMS(DispatcherInstance instance,
			CommandMessage request, String content) throws Exception
	{
		if(request.getParameters().getBoolean("isvt"))
		{
			sendSMSVT(instance, request, request.getServiceAddress(), request.getIsdn(), content);
		}
		else
		{
			sendSMS(instance, request, request.getServiceAddress(), request.getIsdn(), content);
		}
			...
			String content) throws Exception
	{
		...
		try
		{
			...
			// provisioning & command
			CommandEntry command = ProvisioningFactory.getCache().getCommand(
					Constants.COMMAND_SEND_SMSVT);

			requestMO.setProvisioningType(Constants.PROVISIONING_VT8X26);
			requestMO.setCommandId(command.getCommandId());
