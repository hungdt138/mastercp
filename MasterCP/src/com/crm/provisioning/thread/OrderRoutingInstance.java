...
public class OrderRoutingInstance extends ProvisioningInstance
{
	...
	public int processMessage(Message request) throws Exception
	{
		...
		try
		{
			...
			if (orderRoute == null)
			{
				debugMonitor("OrderRoute not found");
			...
			/**
			 * Throws AppException(duplicate request) if the same request is in processing
			 */
...
//			checkProcessingPass = true;
			...
			// get price
			if (!orderRoute.isCheckBalance() && (order.getStatus() != Constants.ORDER_STATUS_DENIED) && order.getParameters().getInteger(Constants.REQUEST_PREFIX + "chargeMode") == 0)
			{
				// set default price
			...
			String content = order.getMsgContent();
			if(content == null)
			{
			...
			if ((orderRoute != null) && orderRoute.isCreateOrder())
			{
				try
				{
					...
					subscriberOrder = SubscriberOrderImpl.createOrder(
							...
							, order.getCause(), order.getStatus(), order.getChannel(), order.getContentType(), order.getContentCode(), order.getOpId(), (int) order.getRequestId(), content, order.getAgentId());
