...
public class ProvisioningCache
{
	...
	protected void loadProvisioning(Connection connection) throws Exception
	{
		...
		try
		{
			...
			// //////////////////////////////////////////////////////
			stmtConfig = connection.prepareStatement("Select * From	ProvisioningEntry Order by code");
			rsConfig = stmtConfig.executeQuery();

			while (rsConfig.next())
			{
				ProvisioningEntry provisioning =
						new ProvisioningEntry(rsConfig.getLong("provisioningId"), rsConfig.getString("code"));

				provisioning.setProvisioningType(Database.getString(rsConfig, "Type"));
	...
	protected synchronized void loadCommand(Connection connection) throws Exception
	{
		...
		try
		{
			...
			// List of Command
			// //////////////////////////////////////////////////////
			stmtConfig = connection.prepareStatement("Select * From CommandEntry Order by Code");
			rsConfig = stmtConfig.executeQuery();

			while (rsConfig.next())
			{
				CommandEntry command = new CommandEntry(rsConfig.getLong("commandId"), rsConfig.getString("Code"));
				command.setAlias(rsConfig.getString("Code"));
				command.setProvisioningType(rsConfig.getString("Type"));
