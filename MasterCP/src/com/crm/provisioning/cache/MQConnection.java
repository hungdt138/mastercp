...
//	}
	...
	public Message sendMessage(CommandMessage request, Queue queue, long timeout) throws Exception
	{
		return sendMessage(request, request.getCorrelationID(), request.getTimeout(), queue, timeout);
	}

	public Message sendMessage(CommandMessage request, String queueName, long timeout) throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, request.getCorrelationID(), request.getTimeout(), queue, timeout);
	}

	public Message sendMessage(Serializable request, String queueName, long timeout) throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, "", 0, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, String correlationId, String queueName, long timeout) throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, correlationId, 0, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, long expiration, String queueName, long timeout) throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, "", expiration, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, String correlationId, long expiration, String queueName, long timeout)
			throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, correlationId, expiration, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, String correlationId, long expiration, String queueName,
			long timeout, String[] propsName, Object[] propsValue) throws Exception
	{
		Queue queue = QueueFactory.getQueue(queueName);
		return sendMessage(request, correlationId, expiration, queue, timeout, propsName, propsValue);
	}

	public Message sendMessage(Serializable request, Queue queue, long timeout) throws Exception
	{
		return sendMessage(request, "", 0, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, String correlationId, Queue queue, long timeout) throws Exception
	{
		return sendMessage(request, correlationId, 0, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, long expiration, Queue queue, long timeout) throws Exception
	{
		return sendMessage(request, "", expiration, queue, timeout, null, null);
	}

	public Message sendMessage(Serializable request, String correlationId, long expiration, Queue queue, long timeout)
			throws Exception
	{
		return sendMessage(request, correlationId, expiration, queue, timeout, null, null);
	}
			...
			long timeout, String[] propsName, Object[] propsValue) throws Exception
	{
		...
		return sendMessage(message, queue, timeout);
	}

	public Message sendMessage(Message message, Queue queue) throws Exception
	{
		try
		{
			QueueFactory.sendMessage(queueSession, queue, message);
		}
		catch (Exception e)
		{
			hasError = true;
	...
	public Message sendMessage(Message message, Queue queue, long timeout) throws Exception
	{
		try
		{
	...
	public Message sendMessage(Message message, String queueName) throws Exception
	{
		try
		{
			QueueFactory.sendMessage(queueSession, queueName, message);
		}
		catch (Exception e)
		{
			hasError = true;
	...
	public Message sendMessage(Message message, String queueName, long timeout) throws Exception
	{
		try
		{
			Queue queue = QueueFactory.getQueue(queueName);
