...
public class WSSecurity
{
	...
	public WSSecurity() {
		try
		{
			int index = WSConfiguration.getConfiguration().getUserFilePath()
					.lastIndexOf("/");

			if (index > 0)
			{
				String directory = WSConfiguration.getConfiguration()
						.getUserFilePath().substring(0, index);
			...
			File f_con = new File(WSConfiguration.getConfiguration()
					.getUserFilePath());
			if (!f_con.exists())
			{
	...
	public void loadUsers()
	{
		try
		{
			...
			File file = new File(WSConfiguration.getConfiguration()
					.getUserFilePath());
			// File file = new File("D:/users.xml");
			WSConfiguration.debugMonitor("Load users from " + file
					.getAbsolutePath());
			...
			for (int i = 0; i < children.getLength(); i++)
			{
				...
				if (child instanceof Element)
				{
					...
					users.add(user);
					WSConfiguration
							.debugMonitor("loadUsers: " + user.getUserName() + ":" + user
									.getPassword() + ":" + user.Permissions
									.size());
				}
			}
	...
	public User authenticate(String username, String password)
	{
		for (int i = 0; i < this.users.size(); i++)
		{
			...
			if (user.getUserName().equalsIgnoreCase(username) && user
