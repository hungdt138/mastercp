...
public class SubscriberOrderImpl
{
	...
	public static SubscriberOrder createOrder(
			...
			, String cause, int status, String channel, String serviceAddr)
			throws Exception
	{
		SubscriberOrder order = null;
		...
		try
		{
			...
			String SQL = "Insert into SubscriberOrder "
					...
					+ "		, offerPrice, price, quantity, discount, amount, score, status, cause, channel,serviceaddr) "
					+ " Values "
					+ "		(?, ?, ?, sysDate, sysDate, ?, ?, ? "
					+ "		, ?, ?, ?, ?, ? "
					+ "		, ?, ?, ?, ?, ?, ?, ?, ?, ?,?) ";
	...
	public static SubscriberOrder createOrder(
			...
			, long subscriberId, String isdn, int subscriberType, long subProductId, long productId
			, double price, int quantity, double discount, double amount, double score
			, String cause, int status, String serviceAddr)
			throws Exception
	{
		return createOrder(userId, userName, orderDate, orderType, subscriberId, isdn, subscriberType
					, subProductId, productId, price, quantity, discount, amount, score, cause, status, "", serviceAddr);
			...
			throws Exception
	{
		...
		try
		{
			...
			return createOrder(
					...
					, subProductId, productId, price, quantity, discount, amount, score, cause, status, channel, serviceAddr);
		}
		finally
		{
			Database.closeObject(connection);
...
//			{
//				Database.processAlarmDB(startTime, endTime, orderDate, productId, isdn, "Insert Order");
//			}
		}
	}
	...
	public static double getChargingMoney(String isdn, String table, Date startDate) throws Exception
	{
		double money = 0;
		Connection connection = null;
		...
		try
		{
			...
			String sql = "select FACE_VALUE from "
							+ table
