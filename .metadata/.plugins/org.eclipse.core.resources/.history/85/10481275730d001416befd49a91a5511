...
public class QuizOrderRouting extends VNMOrderRoutingImpl
{
			...
			ProductRoute orderRoute, CommandMessage order) throws Exception
	{
		...
		try
		{
			...
			String cmdCode = product.getParameters().getString("vt8x26.commandcode");
					...
					.equals("unregister"))
			{
				...
				List<ContentQuestion> lstQId = SubscriberOrderImpl.getListQuestionIdFromOrder(order.getIsdn());
				if (lstQId.size() >= 15)
				{
