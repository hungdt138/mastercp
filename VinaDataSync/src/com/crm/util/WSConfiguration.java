...
public class WSConfiguration extends AppProperties
{
	...
	super();
		...
		try
		{
			file = new File("vinaphone_service/ServerConfig.txt");
			debugMonitor("Load ServerConfig.txt: " + file.getAbsolutePath());
		...
		{
			configStream = WSQueueFactory.class.getResourceAsStream("ServerConfig.txt");
			try
			{
				load(configStream);
			}
			...
			debugMonitor("Load ServiceConfig.txt error: " + file.getAbsolutePath());
			debugMonitor(e);
		}
		finally
		{
