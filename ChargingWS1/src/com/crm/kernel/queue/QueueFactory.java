...
public class QueueFactory
{
	...
	public static MQConnection getConnection() throws Exception
	{
		...
		synchronized (mutex)
		{
			...
			{
				queueConnectionPool = new MQConnectionPool(queueConnectionPoolSize);
			}
	...
	public static QueueSession getSession() throws Exception
	{
		...
		try
		{
			...
			return connection.getQueueConnection().createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
		}
		catch (Exception e)
		{
			if (connection != null)
	...
	public static Message sendMessage(Object object, String queueName) throws Exception
	{
		return sendMessage(object, queueName, 0);
	}

	public static Message sendMessage(Object object, String queueName, long timeout) throws Exception
	{
	...
	public static Message sendMessage(
			Object object, Queue queue, String correlationId
			, String[] propsName, Object[] propsValue, long timeout) throws Exception
	{
		QueueSession session = null;
		...
		try
		{
			...
			return sendMessage(session, object, queue, correlationId, propsName, propsValue, timeout);
		}
		finally
		{
			closeQueue(session);
	...
	public static Message createObjectMessage(Serializable content) throws Exception
	{
		QueueSession session = null;
		...
		synchronized (mutex)
		{
			...
			try
			{
				if (context == null)
				{
					...
					configProvider.loadFromFile(WSConfiguration.configPath + "ServerConfig.txt");
					...
					properties.load(new FileInputStream(WSConfiguration.configPath + "jndi.properties"));

					String host = properties.getProperty("org.omg.CORBA.ORBInitialHost");
					String port = properties.getProperty("org.omg.CORBA.ORBInitialPort");
